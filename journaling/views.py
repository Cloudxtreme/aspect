...
def report_plans(request):
	data = Service.objects.values('plan__speed_in').filter(plan__tos__id=1).order_by('plan__speed_in').annotate(Count('plan__speed_in'))
...
def report_paysbymonth(request):
	data = Payment.objects.extra({'weekday': "dayofmonth(date)"}).values('weekday').order_by('weekday').annotate(Count('id'))
...
def report_paysbyweek(request):
	data = Payment.objects.extra({'weekday': "dayofweek(date)" }).values('weekday').order_by('weekday').annotate(Count('id'))
...
def report_sumbymonth(request):
	...
	data = Payment.objects.extra(select={'month': 'extract( month from date )',
